...
ServerFile = open('Server File.txt', "w")
...
def Main():
    ...
    while True:
        ...
        if file_name == '$':  # Check ServerFile for this file
            ServerFile.close()
            CheckServer = open('Server File.txt', 'r')


            Check = CheckServer.read()
            print(Check)
            sizeCheck = len(Check)
            sizeCheck = bin(sizeCheck)[2:].zfill(16)
            c.send(sizeCheck)
            time.sleep(2)
            c.send(Check.encode())

            # print("List is: "+str(Check))
            ...
            if cnt_down is False:
                ...
                if file_name not in index:   # CHANGE FOR FILE TXT
                    ServerFile.write(str(listFiles))
                ServerFile.close()
            ...
            if cnt_down is True:  # When server has to download to client
                file_name += '#'
